...
const normalize = (beverage: RawData): any => {
  ...
  const formattedObject = {
    ...
    brand: {
      ...
      ...(label('general.brand.consortium') && {
        consortium: label('general.brand.consortium'),
      }),
      shortId: label('general.brand.shortId'),
    },
    cooperation: {
      ...(!isEmpty(label('general.cooperation')) && {
        label: label('general.cooperation').map(({ name }) => name),
      }),
      ...(!isEmpty(producer('general.cooperation')) && {
        producer: producer('general.cooperation').map(({ name }) => name),
      }),
      ...(!isEmpty(editorial('general.cooperation')) && {
        editorial: editorial('general.cooperation').map(({ name }) => name),
      }),
    },
    contract: {
      ...(!isEmpty(label('general.contract')) && {
        label: label('general.contract.name'),
      }),
      ...(!isEmpty(producer('general.contract')) && {
        producer: producer('general.contract.name'),
      }),
      ...(!isEmpty(editorial('general.contract')) && {
        editorial: editorial('general.contract.name'),
      }),
    },
    isContract: {
    ...
    place: {
      ...(label('general.place.city') && {
        label: {
          city: label('general.place.city'),
          country: label('general.place.country.name'),
        },
      }),
      ...(producer('general.place.city') && {
        producer: {
          city: producer('general.place.city'),
          country: producer('general.place.country.name'),
        },
      }),
      ...(editorial('general.place.city') && {
        editorial: {
          city: editorial('general.place.city'),
          country: editorial('general.place.country.name'),
        },
      }),
    },
    remark: {
    ...
    dryHopped: {
      ...(!isEmpty(label('brewing.dryHopped.hops')) && {
        label: label('brewing.dryHopped.hops').map(({ name }) => name),
      }),
      ...(!isEmpty(producer('brewing.dryHopped.hops')) && {
        producer: producer('brewing.dryHopped.hops').map(({ name }) => name),
      }),
      ...(!isEmpty(editorial('brewing.dryHopped.hops')) && {
        editorial: editorial('brewing.dryHopped.hops').map(({ name }) => name),
      }),
    },
    hopRate: {
    ...
    ingredientsList: {
      ...(!isEmpty(label('ingredients.list')) && {
        label: label('ingredients.list'),
      }),
      ...(!isEmpty(producer('ingredients.list')) && {
        producer: producer('ingredients.list'),
      }),
    },
    smokedMalt: {
